 ...
 * storage array.
 */
void _dynarray_resize(struct dynarray* da, int new_capacity, void* dummy) {
  ...
  for (int i = da->size; i < new_capacity; i++) {
    new_data[i] = dummy;
  }
...
struct dynarray* dynarray_create_c(int c) {
  struct dynarray* da = malloc(sizeof(struct dynarray));
  assert(da);

  da->data = malloc(c * sizeof(void*));
  assert(da->data);
  da->size = 0;
  da->capacity = c;
...
void dynarray_insert(struct dynarray* da, void* val) {
  ...
  if (da->size == da->capacity) {
    ...
    int dummy = -1;
    _dynarray_resize(da, 2 * da->capacity, &dummy);
  }
