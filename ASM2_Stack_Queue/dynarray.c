...
//my functions
/*
 * This function returns the start of a given dynamic array.
 */
int dynarray_start(struct dynarray* da) {
  assert(da);
  return da->start;
}
void dynarray_set_start(struct dynarray* da, int start) {
  assert(da);
  da->start = start;
}

void array_enqueue(struct dynarray* da, void* val){
  if (da->size >= da->capacity) {
    //need to reallocate a new array with doubled capacity
    void** new_data = malloc(da->capacity * 2 * sizeof(void*));
    //note: reindex needed here, i.e., logical idx 0 will be put at physical idx 0
    for (int i = 0; i < da->size; i++) {
  ...
  //otherwise, simply insert the element 
  da->data[(da->start + da->size)%da->capacity] = val; //where idx = (start + size) % capacity 
  da->size++; 
}

void* array_dequeue(struct dynarray* da){
  void* val = da->data[da->start];
  //set data[start] to NULL
  da->data[da->start] = NULL;
  //update the start
  //printf("old start %d\n", da->start);
  da->start = (da->start + 1)%da->capacity; 
  //printf("new start %d\n", da->start);
  //note that here you don't need to shift the remaining elements, as remove from queue should have O(1) runtime complexity
  da->size--;
  return val;
}